...
*Versão 1.9 — agosto/2026. Este documento dá contexto completo a qualquer nova conversa. Atualizar ao fim de sessões que mudem decisões, arquitetura ou estado.*
...
- **Plano B (cliente final sem WhatsApp da Empresa), construído:** página pública `app/b/[slug]/page.tsx` (fora do layout autenticado, `empresas` ganhou política de leitura pública `empresas_select_public` pra isso funcionar) com `FormularioClienteFinal` (nome, WhatsApp, e-mail → `signInWithOtp`, reaproveitando a mesma infra de magic link de `/entrar`). O vínculo cliente↔empresa (tabela nova `empresa_clientes`) só é criado depois que o cliente confirma o e-mail — o formulário guarda os dados em `localStorage` (`lib/empresa-clientes.ts`) antes de disparar o magic link, e `components/app/FinalizarCadastroClienteEmpresa.tsx` (montado em `app/app/layout.tsx`, roda em toda página autenticada) lê esse pendente, grava o vínculo e preenche `profiles.phone` se ainda estiver vazio. Painel da Empresa ganhou `CardLinkEmpresa` com o link `/b/slug` pronto pra copiar. Script `docs/sql/plano-b-cliente-final.sql`.
- **Boas-vindas + senha opcional pro cliente final, construído:** o cadastro via `/b/slug` é só magic link (sem senha), o que criava risco do cliente final voltar em `/entrar`, cair na aba de e-mail+senha por padrão e travar sem achar a opção certa. `FinalizarCadastroClienteEmpresa` agora mostra, na primeira vez que esse cliente entra (logo depois de confirmar o magic link), um cartão de boas-vindas convidando a conhecer o resto do Quintalzim, com a opção — não obrigatória — de criar senha ali mesmo (`supabase.auth.updateUser({ password })`, mesma chamada usada em "Alterar senha" no Perfil). Quem recusa continua entrando só por magic link, sem fricção extra
